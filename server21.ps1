	...
	function StartMessageReceiver ($username)
	{
		...
		$message_receive_loop =
		{
			...
			While ($server_state['active']) {
				...
				if ($client.Connected -and $stream.socket.Connected)
				{
...
{
	[console]::writeline("checking remove clients")
	CheckRemoveClients
	[console]::writeline("checking remove clients... Done")
	...
	foreach ($user in $clients.keys)
	{
		...
		if (-not $client.connected)
		{
			...
			[console]::writeline("$client dead, not broadcasting, adding to remove queue")
			continue
		}
		...
		{
			$remove_client_queue.Enqueue($user)
			[console]::writeline("broadcast stream dead, not broadcasting, adding to remove queue")
			continue
		}
