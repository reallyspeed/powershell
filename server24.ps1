				...
				if ($client.Connected)
				{
					if ($stream.DataAvailable)
					{
						...
						If ($return -gt 0)
						{
							...
							$client_message_queue.Enqueue($message) #username, message
						} Else {       
...
function RemoveClient($remove_user)
{
	...
	if ($clients[$remove_user].Connected)
	{
		if ($clients[$remove_user].GetStream().Socket.Connected) { $clients[$remove_user].GetStream().Close() }
...
function BroadcastMessage ($message)
{
	...
	foreach ($user in $clients.keys)
	{
		...
		#try{
			#$d = [BitConverter]::GetBytes([int16]11)
			...
			[console]::writeline("Message_size conv: $message_size_bytes")
